...
class TexLatexDocClsMacro(WikiMacroBase):
    r'''This macro inform to Trac2Latex parser the document class and parameters.

    Ex.:
    {{{
    [[LatexDocClsMacro(atricle,a4paper)]]
    }}}
    '''
    def save_file(self, dir_path, url, server):
        ...
        os.unlink(name)
        urlretrieve(basejoin(host, url + ".cls?format=raw"), name + ".cls")
        return name
    ...
    def expand_macro(self, formatter, name, content):
        try:
            ...
            if pkgargs:
                return  "\\documentclass[%s]{%s}" % (pkgargs, package)
            else:
                return  "\\documentclass{%s}" % package
